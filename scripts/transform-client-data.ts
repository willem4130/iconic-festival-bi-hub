...
function validatePicks(picks: ClientPick[], locationCodes: Set<string>): void {
  ...
  if (invalidPicks.length > 0) {
    console.warn(`⚠ Warning: ${invalidPicks.length} picks reference unknown locations`)
...
function validateArticles(articles: ClientArticle[], locationCodes: Set<string>): void {
  ...
  if (invalidArticles.length > 0) {
    console.warn(`⚠ Warning: ${invalidArticles.length} articles reference unknown pick locations`)
