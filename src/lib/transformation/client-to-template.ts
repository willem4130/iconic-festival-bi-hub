 ...
 */
export function transformLocations(locations: ClientLocation[]): TemplateLocation[] {
  ...
  bayGroups.forEach((bayLocations, bayCode) => {
    ...
    const capacities = bayLocations.map((loc) => capacityMap.get(loc.Location) || 0)
...
export async function transformClientDataToTemplate(
  ...
  const articleLocations = transformArticleLocations(articlesData, locationsData)
